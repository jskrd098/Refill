...
FILENAME = 'bible_monthly_r2.pdf' # ファイル名
year = 2025 # 出力したい年
...
def DrawWeekDay(pdf, w, h, wd):
    ...
    if wd == 6:
        pdf.setFillColorRGB(1, 0.5, 0.5) # 日曜日を赤文字に
    elif wd == 5:
        pdf.setFillColorRGB(0.5, 0.5, 1) # 土曜日を青文字に 
    else:
        pdf.setFillColorRGB(0.6, 0.6, 0.6)
...
def main():
    ...
    ## 最初のページ
    # 年間カレンダー
    DrawYearCalendar(pdf, nowYear)
    pdf.showPage() # 改ページ

    for month in range(1, 16): # 1月～来年3月
        
        nowMonth = (month-1)%12+1
        
        # データの取得
        cal_list = calendar.monthcalendar(nowYear, nowMonth)
        ...
        # 月～木
        for wd in range(0, 4):
            DrawWeekDay(pdf, block_w*wd+x, 0, wd)
        for w in range(0, len(cal_list)):
            for wd in range(0, 4):
                DrawDay(pdf, block_w*wd+x, weekday_h+block_h*w, nowYear, nowMonth, cal_list[w][wd], wd)
        pdf.showPage() # 改ページ
        ...
        # 金～日
        for wd in range(4, 7):
            DrawWeekDay(pdf, block_w*(wd-4)+ringhole, 0, wd)
        for w in range(0, len(cal_list)):
            for wd in range(4, 7):
                DrawDay(pdf, block_w*(wd-4)+ringhole, weekday_h+block_h*w, nowYear, nowMonth, cal_list[w][wd], wd)
        
        # 月の見出し
        DrawYearMonth(pdf, x-3*mm+block_w*(wd-3)+ringhole, nowYear, nowMonth)
        ...
        if nowMonth == 1:
            ...
            nextYear = nowYear+1
            nextMonth = 1
        DrawMiniCalendar(pdf, x-2*mm+block_w*(wd-3)+ringhole, size_h-42*mm, 2.7*mm, 2*mm, 5, prevYear, prevMonth)
        DrawMiniCalendar(pdf, x-2*mm+block_w*(wd-3)+ringhole, size_h-24*mm, 2.7*mm, 2*mm, 5, nextYear, nextMonth)
